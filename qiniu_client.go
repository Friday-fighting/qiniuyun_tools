...
func NewClient(accessKey, secretKey, urlPrefix, bucket, expTimeKey string, expTime int) *Client {
	if expTime <= 0 {
		expTime = 3600
	}
	return &Client{
		...
		SecretKey:  secretKey,
		UrlPrefix:  urlPrefix,
		ExpTimeKey: expTimeKey,
		ExpTime:    expTime,
		Bucket:     bucket,
	}
}

func (client *Client) SaveFile(filaName string, fileData []byte) (string, error) {
...
func (client *Client) ReStoreURl(req *UnixTimeReq) string {
	path := req.Path
	...
	expTimeHex := fmt.Sprintf("%x", time.Now().Unix()+int64(client.ExpTime))
	s := fmt.Sprintf("%s%s%s", client.ExpTimeKey, url.QueryEscape(path), expTimeHex)
	m := md5.New()
	m.Write([]byte(s))
	sign := hex.EncodeToString(m.Sum(nil))
	qiNiuUrl := client.UrlPrefix
...
func (client *Client) GenToken(key string) (token string, err error) {
	if key == "" {
